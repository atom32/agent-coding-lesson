...
import { LEARNING_PATH, getVersionMeta } from "@/lib/constants";
...
export default function ComparePage() {
  ...
  const metaA = versionA ? getVersionMeta(versionA, locale) : null;
  const metaB = versionB ? getVersionMeta(versionB, locale) : null;
          ...
          >
            ...
            {LEARNING_PATH.map((v) => (
              <option key={v} value={v}>
                {v} - {tSession(v) || getVersionMeta(v, locale)?.title}
              </option>
            ))}
          </select>
        </div>
      ...
      {infoA && infoB && comparison && (
        <div className="space-y-8">
          ...
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
            <Card>
              <CardHeader>
                <CardTitle>{versionA ? tSession(versionA) : metaA?.title || versionA}</CardTitle>
            ...
            <Card>
              <CardHeader>
                <CardTitle>{versionB ? tSession(versionB) : metaB?.title || versionB}</CardTitle>
            ...
            <div className="grid grid-cols-1 gap-4 lg:grid-cols-2">
              <div>
                <h3 className="mb-3 text-sm font-medium text-zinc-500 dark:text-zinc-400">
                  {versionA ? tSession(versionA) : metaA?.title || versionA}
                </h3>
                <ArchDiagram version={versionA} />
              </div>
              <div>
                <h3 className="mb-3 text-sm font-medium text-zinc-500 dark:text-zinc-400">
                  {versionB ? tSession(versionB) : metaB?.title || versionB}
                </h3>
            ...
            <div className="grid grid-cols-1 gap-6 sm:grid-cols-3">
              <div>
                <h4 className="mb-2 text-sm font-medium text-zinc-600 dark:text-zinc-400">
                  {t("only_in")} {versionA ? tSession(versionA) : metaA?.title || versionA}
                </h4>
              ...
              <div>
                <h4 className="mb-2 text-sm font-medium text-zinc-600 dark:text-zinc-400">
                  {t("only_in")} {versionB ? tSession(versionB) : metaB?.title || versionB}
                </h4>
                {comparison.toolsOnlyB.length === 0 ? (
                  <p className="text-xs text-zinc-400">{t("none")}</p>
                ) : (
