...
import { LEARNING_PATH, VERSION_META, LAYERS, getVersionMeta } from "@/lib/constants";
...
export default function HomePage() {
  ...
  return (
    <div className="flex flex-col gap-20 pb-16">
      ...
      {/* Double-layer Curriculum */}
      <section>
        <div className="mb-6 text-center">
          <h2 className="text-2xl font-bold sm:text-3xl">
            {t("double_layer_title")}
          </h2>
          <p className="mx-auto mt-2 max-w-2xl text-[var(--color-text-secondary)]">
            {t("double_layer_desc")}
          </p>
        </div>
        <div className="grid gap-4 md:grid-cols-[1fr_auto_1fr] md:items-stretch">
          <div className="rounded-lg border border-blue-200 bg-blue-50 p-5 dark:border-blue-900/70 dark:bg-blue-950/30">
            ...
            <h3 className="mt-3 text-lg font-semibold text-blue-950 dark:text-blue-50">
              {t("lab_track_title")}
            </h3>
            <p className="mt-2 text-sm leading-relaxed text-blue-950/75 dark:text-blue-100/75">
              {t("lab_track_desc")}
            </p>
          </div>
          <div className="hidden items-center px-2 text-zinc-300 md:flex">
            <ArrowRight size={24} />
          </div>
          <div className="rounded-lg border border-emerald-200 bg-emerald-50 p-5 dark:border-emerald-900/70 dark:bg-emerald-950/30">
            ...
            <h3 className="mt-3 text-lg font-semibold text-emerald-950 dark:text-emerald-50">
              {t("production_track_title")}
            </h3>
            <p className="mt-2 text-sm leading-relaxed text-emerald-950/75 dark:text-emerald-100/75">
              {t("production_track_desc")}
            </p>
          </div>
        </div>
        <div className="mt-4 text-center">
          <Link
            href={`/${locale}/curriculum`}
            className="inline-flex min-h-[40px] items-center gap-2 rounded-md border border-zinc-200 px-4 text-sm font-medium text-zinc-700 transition-colors hover:bg-zinc-50 dark:border-zinc-700 dark:text-zinc-300 dark:hover:bg-zinc-800"
          >
            <Map size={16} />
            {t("open_course_map")}
          </Link>
      ...
      <section>
        ...
        <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-3">
          {LEARNING_PATH.map((versionId) => {
            const meta = getVersionMeta(versionId, locale);
      ...
      <section>
        <div className="mb-6 text-center">
          <h2 className="text-2xl font-bold sm:text-3xl">
            {t("bridge_preview_title")}
          </h2>
          <p className="mt-2 text-[var(--color-text-secondary)]">
            {t("bridge_preview_desc")}
          </p>
        </div>
        <div className="grid grid-cols-1 gap-3 lg:grid-cols-2">
          {COURSE_BRIDGES.slice(0, 6).map((rawBridge) => {
            const bridge = localizeBridge(rawBridge, locale);
            const meta = getVersionMeta(bridge.version, locale);
            const firstChapter = localizeChapter(
              ADVANCED_CHAPTERS[bridge.chapters[0]],
              locale
            );
            return (
              <Link
                ...
                className="group rounded-lg border border-[var(--color-border)] bg-[var(--color-bg)] p-4 transition-colors hover:bg-[var(--color-bg-secondary)]"
              >
                <div className="flex flex-wrap items-center gap-2">
                  ...
                  <span className="text-sm text-zinc-600 dark:text-zinc-300">
                    EP{firstChapter.id} {firstChapter.title}
                  </span>
                </div>
              ...
              <div className="flex-1">
                ...
                <div className="mt-2 flex flex-wrap gap-1.5">
                  {layer.versions.map((vid) => {
                    const meta = getVersionMeta(vid, locale);
                    return (
