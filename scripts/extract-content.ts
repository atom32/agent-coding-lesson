...
const AGENTS_DIR = path.join(RESOURCES_DIR, "learn-claude-code", "agents");
const DOCS_DIR = path.join(RESOURCES_DIR, "learn-claude-code", "docs");
const ADVANCED_DOCS_DIR = path.join(RESOURCES_DIR, "claude-reviews-claude", "docs");
const ADVANCED_ASSETS_DIR = path.join(ADVANCED_DOCS_DIR, "assets");
const OUT_DIR = path.join(WEB_DIR, "src", "data", "generated");
const PUBLIC_ADVANCED_ASSETS_DIR = path.join(WEB_DIR, "public", "advanced-assets");
...
function extractAdvancedChapterId(filename: string): string | null {
...
function main() {
  ...
  console.log(`  Advanced docs dir: ${ADVANCED_DOCS_DIR}`);
  ...
  // 5. Read advanced Claude Code review docs from the sibling repository.
  const advancedDocs: {
    chapter: string;
    locale: "en" | "zh";
    title: string;
  ...
  if (fs.existsSync(ADVANCED_DOCS_DIR)) {
    const advancedLocaleDirs = [
      { locale: "en" as const, dir: path.join(ADVANCED_DOCS_DIR, "chapters") },
      { locale: "zh" as const, dir: path.join(ADVANCED_DOCS_DIR, "zh-CN", "chapters") },
    ];
    let totalAdvancedFiles = 0;

    for (const { locale, dir } of advancedLocaleDirs) {
      if (!fs.existsSync(dir)) continue;
      const files = fs.readdirSync(dir).filter((f) => f.endsWith(".md"));
      totalAdvancedFiles += files.length;

      for (const filename of files) {
        const chapter = extractAdvancedChapterId(filename);
        if (!chapter) {
          console.warn(`  Skipping advanced doc ${locale}/${filename}: could not determine chapter`);
          continue;
        }
        ...
        const titleMatch = content.match(/^#\s+(.+)$/m);
        const title = titleMatch ? titleMatch[1] : filename;
        advancedDocs.push({ chapter, locale, title, filename, content });
      }
    }

    copyDir(ADVANCED_ASSETS_DIR, PUBLIC_ADVANCED_ASSETS_DIR);
    console.log(`  Found ${totalAdvancedFiles} advanced doc files`);
    console.log(`  Copied advanced assets to ${PUBLIC_ADVANCED_ASSETS_DIR}`);
  } else {
    console.warn(`  Advanced docs directory not found: ${ADVANCED_DOCS_DIR}`);
  }
  ...
  const advancedDocsPath = path.join(OUT_DIR, "advanced-docs.json");
  fs.writeFileSync(advancedDocsPath, JSON.stringify(advancedDocs, null, 2));
  console.log(`  Wrote ${advancedDocsPath}`);
  ...
  console.log(`  ${advancedDocs.length} advanced docs`);
